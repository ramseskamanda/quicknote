	...
	case tea.KeyMsg:
		...
		case key.Matches(msg, m.keymap.quit):
			for _, input := range m.inputs {
				if err := m.onSave(input.Value()); err != nil {
		...
		case key.Matches(msg, m.keymap.save):
			if err := m.onSave(m.inputs[m.focus].Value()); err != nil {
		...
		case key.Matches(msg, m.keymap.remove):
			...
			m.inputs = append(m.inputs[:m.focus], m.inputs[m.focus+1:]...)
			if m.focus > len(m.inputs)-1 {
				m.focus = len(m.inputs) - 1
			}
...
func (m EditorModel) sizeInputs() {
	v, h := focusedBorderStyle.GetFrameSize()

	for i := range m.inputs {
		m.inputs[i].SetWidth(m.width/len(m.inputs) - h)
		m.inputs[i].SetHeight(m.height - helpHeight - v)
	}
}
